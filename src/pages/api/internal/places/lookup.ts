    ...
    return Response.json(
      {
        error:
          status === 400 || status === 422
            ? "Der Google-Maps-Link konnte nicht gelesen werden. Verwenden Sie den Teilen-Link des Unternehmensprofils."
            : "Google Places war nicht erreichbar. Bitte versuchen Sie es später erneut.",
      },
      { status, headers },
    );
  }
